...
var deploymentStopCmd = &cobra.Command{
	...
	Short: "Stops one or more deployments that matches the given pattern",
	Long: `Stops one or more deployments, that mathcer the given pattern, in a specific organization and environment. 
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		for _, d := range payload.Items {
			if pattern.Match([]byte(d.Name)) {
				...
				if err != nil {
					log.Fatalf("Failed to stop %s : %+v", d.Name, err)
				}
				log.Printf("Stoping %s\n", d.Name)
				if viper.GetBool("deploymentstopwait") {
...
func init() {
	deploymentCmd.AddCommand(deploymentStopCmd)

	deploymentStopCmd.Flags().BoolP("wait", "w", false, "Wait for the deployments to fully start before continuing")
	viper.BindPFlag("deploymentstopwait", deploymentStopCmd.Flags().Lookup("wait"))
}
